...
It contains 15 categories in a multi-level hierarchy and 200 realistic Kenyan retail products with EAN-13
test barcodes, SKU values, KES buying and selling prices, units, expiry flags,
and 20 promotional prices for checkout testing. It also configures three stores,
three suppliers, 12 supplier catalog/package records, nine store reorder policies,
and five purchase orders demonstrating completed, partially received, issued, and
draft states. Accepted demo receipts create store-specific inventory, including
short-life lots expiring today and soon. Completed, dispatched/in-transit, and
draft transfers demonstrate stock movement between branches. The loader never
creates anonymous opening stock.

Re-running the loader updates catalog, store, and supplier records by stable codes
and reuses marked purchase orders and transfers instead of intentionally adding
more demo documents. Validate the full specification without AWS access:
