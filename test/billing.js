...
assert.equal(PLANS.biashara.monthlyPriceKes, 500);
assert.equal(PLANS.biashara_growth.monthlyPriceKes, 1500);
...
const customPlan = effectivePlan(account({ override: { activeUserLimit: 8, monthlyPriceKes: 1200, reason: "Custom", updatedAt: "2026-08-01", updatedBy: "admin" } }));
assert.equal(customPlan.activeUserLimit, 8);
assert.equal(customPlan.monthlyPriceKes, 1200, "an active negotiated price remains authoritative");
const existingRenewal = nextBillingPayment(account({ planCode: "biashara", paidThrough: "2026-09-30" }), "2026-09-15");
assert.equal(existingRenewal.amountKes, 500, "existing standard-plan accounts receive the current price on their next calculated renewal");
assert.equal(existingRenewal.periodStartsOn, "2026-10-01", "the price change does not shorten the current paid period");
...
assert.equal(trialCharge.amountKes, 1500);
...
assert.equal(creditedCharge.cashDueKes, 750);
...
assert.equal(annualCharge.baseAmountKes, 18000);
assert.equal(annualCharge.amountKes, 16200, "annual billing charges 90% of twelve monthly payments");
assert.equal(annualCharge.savingsKes, 1800);
assert.equal(annualCharge.annualDiscountKes, 1800);
...
assert.equal(annualWithOffer.amountKes, 5400, "annual savings do not stack with monthly promotions");
...
assert.equal(annualPromotionCharge.baseAmountKes, 18000);
assert.equal(annualPromotionCharge.annualDiscountKes, 1800, "the standard annual discount remains a separate credit");
assert.equal(annualPromotionCharge.promotionCreditKes, 4860, "the promotion is applied after the annual discount");
assert.equal(annualPromotionCharge.amountKes, 11340);
assert.equal(nextBillingPayment(account({ offer: { ...launchOffer, remainingPayments: 0 } }), "2026-08-04").amountKes, 500, "a consumed offer no longer changes the charge");
