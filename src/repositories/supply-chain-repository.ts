...
export interface TransferLineRecord {
  ...
  stockUnit: string;
  quantity: number;
  allocations?: Array<{ lotId: string; quantity: number; unitCost: number; costMinor?: number; batchNumber: string; expiryDate?: string | null; supplierId?: string | null }>;
}
...
export const lotRemainingCostMinor = (lot: InventoryLotRecord) => lot.remainingCostMinor ?? Math.round(lot.remainingQuantity * lot.unitCost * 100);
export const allocatedLotCostMinor = (lot: InventoryLotRecord, quantity: number) => quantity === lot.remainingQuantity
  ? lotRemainingCostMinor(lot)
  : Math.round(lotRemainingCostMinor(lot) * quantity / lot.remainingQuantity);
const lotCostAfterCount = (lot: InventoryLotRecord, nextQuantity: number) => nextQuantity >= lot.remainingQuantity
  ? lotRemainingCostMinor(lot) + Math.round((nextQuantity - lot.remainingQuantity) * lot.unitCost * 100)
...
export const upsertSupplierProduct = async (tenantId: string, input: Omit<SupplierProductRecord, "unitsPerPurchaseUnit" | "updatedAt">) => {
  ...
  if (!input.productUnitId && !normalized(input.purchaseUnit)) throw new Error("Purchase unit is required");
  ...
  const record: SupplierProductRecord = selectedProductUnit
    ? { ...input, productUnitId: selectedProductUnit.id, unitsPerPurchaseUnit, supplierSku: normalizedCode(input.supplierSku), purchaseUnit: selectedProductUnit.name, purchaseQuantity: unitsPerPurchaseUnit, purchaseMeasurementUnit: product.baseUnit, updatedAt: now }
    : { ...input, productUnitId: null, unitsPerPurchaseUnit, supplierSku: normalizedCode(input.supplierSku), purchaseUnit: normalized(input.purchaseUnit).toLowerCase(), purchaseMeasurementUnit: input.purchaseMeasurementUnit.trim().toLowerCase(), updatedAt: now };
  if (!selectedProductUnit && !PURCHASE_UNITS[product.baseUnit]?.has(record.purchaseUnit.toLowerCase())) throw new Error(`${record.purchaseUnit} is not a supported purchase package for ${product.name}`);
...
export const storeStock = async (tenantId: string, storeId: string) => {
  ...
  for (const lot of lots) {
    ...
    value.quantity += lot.remainingQuantity; value.inventoryValue = roundMoney(value.inventoryValue + lotRemainingCostMinor(lot) / 100); byProduct.set(lot.productId, value);
  }
...
export const allocateLots = async (tenantId: string, storeId: string, requirements: Array<{ productId: string; quantity: number }>) => {
  ...
  const allocations = new Map<string, Array<{ lot: InventoryLotRecord; quantity: number; costMinor: number }>>();
  for (const requirement of requirements) {
    ...
    const selected: Array<{ lot: InventoryLotRecord; quantity: number; costMinor: number }> = [];
    for (const lot of candidates) { if (remaining === 0) break; const quantity = Math.min(remaining, lot.remainingQuantity); if (quantity > 0) selected.push({ lot, quantity, costMinor: allocatedLotCostMinor(lot, quantity) }); remaining -= quantity; }
...
export const lotDecrement = (tenantId: string, lot: InventoryLotRecord, quantity: number, now: string) => {
  ...
  const nextCostMinor = lotRemainingCostMinor(lot) - allocatedLotCostMinor(lot, quantity);
  return { Update: { TableName: TABLE_NAME, Key: key(tenantId, "LOT", lot.id), UpdateExpression: "SET remainingQuantity = remainingQuantity - :quantity, remainingCostMinor = :nextCostMinor, #status = :nextStatus, updatedAt = :now, accessPartition = :nextPartition, accessSort = :nextSort", ConditionExpression: "remainingQuantity >= :quantity AND #status = :active", ExpressionAttributeNames: { "#status": "status" }, ExpressionAttributeValues: { ":quantity": quantity, ":nextCostMinor": nextCostMinor, ":active": "active", ":nextStatus": exhausted ? "exhausted" : "active", ":now": now, ":nextPartition": collection(tenantId, `STORE#${lot.storeId}#INVENTORY#${exhausted ? "LOT" : "ACTIVE"}`), ":nextSort": exhausted ? `${now}#${lot.id}` : `${lot.expiryDate ?? "9999-12-31"}#${lot.receivedAt}#${lot.id}` } } };
};
...
export const countLot = async (tenantId: string, lotId: string, physicalQuantity: number, reason: string, actor: Actor, requestId: string) => {
  ...
  return commitIdempotent(tenantId, "count", requestId, payload, movement, [{ Update: { TableName: TABLE_NAME, Key: key(tenantId, "LOT", lot.id), UpdateExpression: "SET remainingQuantity = :quantity, remainingCostMinor = :remainingCostMinor, #status = :status, updatedAt = :now, accessPartition = :partition, accessSort = :sort", ConditionExpression: "remainingQuantity = :before", ExpressionAttributeNames: { "#status": "status" }, ExpressionAttributeValues: { ":quantity": physicalQuantity, ":remainingCostMinor": lotCostAfterCount(lot, physicalQuantity), ":before": lot.remainingQuantity, ":status": status, ":now": now, ":partition": collection(tenantId, `STORE#${lot.storeId}#INVENTORY#${physicalQuantity ? "ACTIVE" : "LOT"}`), ":sort": physicalQuantity ? `${lot.expiryDate ?? "9999-12-31"}#${lot.receivedAt}#${lot.id}` : `${now}#${lot.id}` } } }, { Put: { TableName: TABLE_NAME, Item: { ...key(tenantId, "MOVEMENT", movement.id, "EVENT"), accessPartition: collection(tenantId, "STOCK#MOVEMENT"), accessSort: `${now}#${movement.id}`, entityType: "stock_movement", tenantId, ...movement } } }]);
};
...
export const dispatchTransfer = async (tenantId: string, id: string, actor: Actor, requestId: string) => {
  ...
  const nextLines = transfer.lines.map((line) => ({ ...line, allocations: allocations.get(line.productId)!.map(({ lot, quantity, costMinor }) => ({ lotId: lot.id, quantity, unitCost: lot.unitCost, costMinor, batchNumber: lot.batchNumber, expiryDate: lot.expiryDate, supplierId: lot.supplierId })) }));
...
export const receiveTransfer = async (tenantId: string, id: string, receiptInput: Array<{ lotId: string; receivedQuantity: number; damagedQuantity: number; missingQuantity: number; reason: string }>, actor: Actor, requestId: string) => {
  ...
  const transaction: NonNullable<TransactWriteCommandInput["TransactItems"]> = [putTransfer(tenantId, next, transfer.updatedAt)];
  for (const allocation of allocations) { const receiptLine = receiptLines.find((line) => line.lotId === allocation.lotId)!; if (receiptLine.receivedQuantity > 0) { const lotId = randomUUID(); receiptLine.destinationLotId = lotId; const allocationCostMinor = allocation.costMinor ?? Math.round(allocation.quantity * allocation.unitCost * 100); const receivedCostMinor = receiptLine.receivedQuantity === allocation.quantity ? allocationCostMinor : Math.round(allocationCostMinor * receiptLine.receivedQuantity / allocation.quantity); const lot: InventoryLotRecord = { id: lotId, storeId: transfer.toStoreId, productId: allocation.productId, productName: allocation.productName, supplierId: allocation.supplierId, receiptId: null, batchNumber: allocation.batchNumber, expiryDate: allocation.expiryDate, receivedQuantity: receiptLine.receivedQuantity, remainingQuantity: receiptLine.receivedQuantity, unitCost: receivedCostMinor / 100 / receiptLine.receivedQuantity, receivedCostMinor, remainingCostMinor: receivedCostMinor, origin: "transfer", status: "active", receivedAt: now, updatedAt: now }; transaction.push({ Put: { TableName: TABLE_NAME, Item: { ...key(tenantId, "LOT", lotId), accessPartition: collection(tenantId, `STORE#${lot.storeId}#INVENTORY#ACTIVE`), accessSort: `${lot.expiryDate ?? "9999-12-31"}#${now}#${lotId}`, entityType: "inventory_lot", tenantId, ...lot }, ConditionExpression: "attribute_not_exists(partitionKey)" } }, stockMovementPut(tenantId, { type: "transfer_receive", storeId: transfer.toStoreId, productId: allocation.productId, productName: allocation.productName, lotId, quantity: receiptLine.receivedQuantity, unitCost: lot.unitCost, reason: `Received ${transfer.transferNumber}`, referenceId: transfer.id, actorId: actor.id, actorName: actor.name }, now)); } if (receiptLine.damagedQuantity > 0) transaction.push(stockMovementPut(tenantId, { type: "transfer_damage", storeId: transfer.toStoreId, productId: allocation.productId, productName: allocation.productName, lotId: allocation.lotId, quantity: -receiptLine.damagedQuantity, unitCost: allocation.unitCost, reason: receiptLine.reason, referenceId: transfer.id, actorId: actor.id, actorName: actor.name }, now)); if (receiptLine.missingQuantity > 0) transaction.push(stockMovementPut(tenantId, { type: "transfer_shortage", storeId: transfer.toStoreId, productId: allocation.productId, productName: allocation.productName, lotId: allocation.lotId, quantity: -receiptLine.missingQuantity, unitCost: allocation.unitCost, reason: receiptLine.reason, referenceId: transfer.id, actorId: actor.id, actorName: actor.name }, now)); }
